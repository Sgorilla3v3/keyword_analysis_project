...
def visualize_cooc_network(input_path, top_k=100, output_path=None):
    ...
    # (2) 노드: degree 기반 크기
    node_size = [300 + 80 * G.degree(n) for n in G.nodes()]
    ...
    # (3) 라벨: degree 높은 노드만 크게 보이도록
    nx.draw_networkx_labels(
        G, pos,
        font_size=9,
        font_family="Malgun Gothic"  # 한글 깨짐 방지
    )
...
if __name__ == "__main__":
    ...
    window_size = 20 # 관계성은 세밀하게 측정하고 많은 토큰을 이미지에 담기위해 윈도우를 코드에서 고정함
